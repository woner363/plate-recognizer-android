...
class CsvExporterTest {
    ...
    @Test fun header_has_bom_and_columns() {
        ...
        assertTrue(csv.contains("ID,车牌号,候选质量分,拍摄时间,图片URI,已修正,备注"))
    }

    @Test fun normal_record_formats_correctly() {
        val r = PlateRecord(
            id = 1, plateNo = "京A12345", qualityScore = 0.95f,
        ...
        assertTrue("候选质量分用 Locale.US 点号: $dataLine", dataLine.contains("0.95"))
        assertTrue("未修正标 否", dataLine.contains(",否,"))
    }

    @Test fun comma_in_note_is_quoted() {
        val r = PlateRecord(
            id = 2, plateNo = "京A12345", qualityScore = 0.5f,
    ...
    @Test fun formula_injection_is_neutralized() {
        val r = PlateRecord(
            id = 3, plateNo = "京A12345", qualityScore = 0.5f,
    ...
    @Test fun corrected_flag_shows_yes() {
        val r = PlateRecord(
            id = 4, plateNo = "京A12345", qualityScore = 0.5f,
    ...
    @Test fun newline_in_note_is_quoted() {
        val r = PlateRecord(
            id = 5, plateNo = "京A12345", qualityScore = 0.5f,
            capturedAt = 0L, imageUri = null, corrected = false,
            note = "line1\nline2",
        )
        val dataLine = CsvExporter.buildCsvInternal(listOf(r)).lineSequence().drop(1).first()
